...
from .smc import VERBOSITY_HIGH, PowerPosteriorParticleDensity, SingleModelMPD
...
class RJMCMC:
    ...
    def single_mutation(self, theta, llh, N):
        ...
        prop_theta[:], prop_lpqratio[:], prop_id[:] = self.pmodel.propose(theta, N)
        ...
        Proposal.setAcceptanceRates(prop_id, log_acceptance_ratio, 1)
...
class RJBridge:
    ...
    def estimate_log_p_mk(self, target_draws, blocksize=None):
        ...
        prop_model_key_dict, prop_reverse_key_ref = self.pmodel.enumerateModels(prop_theta)
        ...
        for i, mk in enumerate(mk_list):
            for j, prop_mk in enumerate(mk_list):
                if mkmklar[(mk, prop_mk)].shape[0] > 0:
                    elar_mat[i, j] = logsumexp(mkmklar[(mk, prop_mk)]) - np.log(mkmklar[(mk, prop_mk)].shape[0])
