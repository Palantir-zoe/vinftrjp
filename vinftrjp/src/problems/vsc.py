...
class TargetConditionalRobustBlockVSModel:
    ...
    def log_prob(self, beta, context):
        ...
        Notes
        -----
        In true Bayesian VS, inactive params often have same prior. The log prior is the sum of the log prior densities
        for all parameters (active and inactive).

        The likelihood is computed only for the active parameters (by zeroing out the inactive ones). This is correct
        because the inactive parameters do not affect the linear predictor. The linear predictor is computed as
        eta = x_data @ beta_active.T (so inactive coefficients do not contribute).

        The prior is computed for all parameters (including inactive ones) and then the likelihood is computed only for
        the active ones. This is consistent with the Bayesian variable selection framework where the prior is on the
        parameters and the likelihood conditions on the active set.

        However, note that the prior on the inactive parameters is the same as the prior on the active ones. This is
        acceptable because the marginal likelihood (or evidence) for the model will account for the fact that inactive
        parameters are integrated out (and here they are not used in the likelihood).

        However, note that the prior on the inactive coefficients is the same as on the active ones, so the model does
        not encourage sparsity. But the variable selection is done by the mask, so we are considering different
        models (masks) and then comparing them. In this case, for a given mask, the inactive coefficients are drawn from
        the prior and then ignored. So the marginal likelihood of the data given the mask (which is the integral over
        the active and inactive coefficients) will have a factor from the inactive coefficients that is the same for all
        masks (because the prior is the same and they don't appear in the likelihood). So when comparing masks, the
        inactive coefficients contribute the same constant factor, so they don't affect the model comparison. This is
        acceptable.
