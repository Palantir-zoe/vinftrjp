from src.vi_nflows import TrainConditionalNormalizingFlow


def get_normalizing_flows(q0, target):
    kwargs = {"max_iter": 10000 * 4, "lr": 1e-4, "num_samples": 2**8 * 4, "device": "cpu", "verbose": True}
    config = {
    ...
    model = TrainConditionalNormalizingFlow(q0=q0, **kwargs).run(target=target, **config)
    return model
